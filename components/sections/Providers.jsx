    ...
    <section id="providers" className="py-20 px-4 bg-background">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <p className="text-accent font-semibold uppercase tracking-widest mb-4">Expert Team</p>
          <h2 className="text-4xl md:text-5xl font-bold text-primary mb-6 text-balance">
            Meet Our Providers
          </h2>
          <p className="text-lg text-foreground/70 max-w-2xl mx-auto text-pretty">
            Our board-certified physicians bring decades of combined experience and genuine compassion to every patient interaction.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {providers.map((provider, idx) => (
            <div key={idx} className="bg-white rounded-lg overflow-hidden border border-secondary hover:shadow-lg transition-all duration-300">
              {/* Image Placeholder */}
              <div className="h-40 bg-gradient-to-br from-accent/20 to-primary/10 flex items-center justify-center">
                <div className="text-center">
                  <div className="w-20 h-20 bg-accent/20 rounded-full mx-auto mb-2" />
                  <p className="text-xs text-primary font-semibold">{provider.name.split(' ')[1]}</p>
                </div>
              </div>

              {/* Content */}
              <div className="p-6">
                <h3 className="font-bold text-primary text-lg mb-1">{provider.name}</h3>
                <p className="text-accent text-sm font-semibold mb-2">{provider.specialty}</p>
                <p className="text-xs text-foreground/60 mb-4">{provider.credentials}</p>
                <p className="text-foreground/70 text-sm leading-relaxed">{provider.bio}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 bg-accent/5 rounded-lg p-8 border border-accent/20 text-center">
          ...
          <p className="text-foreground/70 max-w-2xl mx-auto mb-6">
            All our providers are board-certified, actively involved in continuing medical education, and committed to staying at the forefront of pulmonary and sleep medicine.
          </p>
          <div className="grid sm:grid-cols-3 gap-6">
            <div>
              <p className="text-2xl font-bold text-accent">95+</p>
              <p className="text-sm text-foreground/70">Combined Years Experience</p>
            </div>
            <div>
